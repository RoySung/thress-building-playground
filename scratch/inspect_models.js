    ...
    json.meshes?.forEach(m => {
      m.primitives?.forEach(p => {
        ...
        if (acc && acc.min && acc.max) {
          ...
          console.log(` -> center:`, center.map(v => v.toFixed(3)), `size:`, size.map(v => v.toFixed(3)));
        }
      });
    });
  }
